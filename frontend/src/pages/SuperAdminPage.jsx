...
const SuperAdminPage = () => {
  ...
  const handleUpdateAdmin = async () => {
    try {
      const response = await fetch(`http://localhost:3001/admin/accounts/${editingAdmin.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(adminForm)
      });
  ...
  const handleDeleteAdmin = async (adminId) => {
    if (window.confirm('Are you sure you want to delete this admin account?')) {
      try {
        const response = await fetch(`http://localhost:3001/admin/accounts/${adminId}`, {
          method: 'DELETE'
        });
  ...
  const openServiceModal = (service = null) => {
    if (service) {
      ...
      setServiceForm({
        ...
        is_fast_track_available: service.is_fast_track_available || false,
